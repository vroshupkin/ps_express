...
const step_1: TCourseVideosTuple[] = [[1, 'Почему NodeJS для backend?', 11.34]];
...
function convertObjectToArrCourse(): void {
	...
	for (const course_id of Object.keys(node_js_course)) {
		...
		for (const videosId of Object.keys(node_js_course[course_id_num].videos)) {
			...
			videos.push([videosId_num, video.name, video.time]);
		}
...
function addNewCourse(
	main_obj: ICourseMainObject,
	courseNameTuple: TCourseNameTuple,
	step_arr: TCourseVideosTuple[],
): void {
...
function FactoryCourses(): CourseStep[] {
	const courses: CourseStep[] = [];
 ...
 */
export function printProgress(step: number, step_videos: number): void {
	...
	console.log(
		`Видео ` +
			`${step_videos}/${minutes_arr.length}: ` +
			`'${names_arr[step_videos - 1]}' ` +
 ...
 */
export const helperProgress = (task: number[], complete_ind: number): string => {
	...
	const complete_sum = task.reduce((prev, curr, ind) => {
		if (ind >= complete_ind) return prev;
		return Minute.add(prev, curr);
	}, new Minute('0.00'));
